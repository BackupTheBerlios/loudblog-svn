...
for ($j=1; $j<=$countcat; $j++) {
    ...
    if (isset($_POST[$tempdel])) {
        ...
        if (isset($_POST[$tempcat])) {
            ...
            $putcat = urlencode(html_entity_decode($_POST[$tempcat]));
...
echo checker($row['edit_own']) . " />\n";
echo "<input type=\"checkbox\" disabled=\"disabled\" ";
echo checker($row['publish_own']) . " />\n";
echo "<input type=\"checkbox\" disabled=\"disabled\" ";
echo checker($row['edit_all']) . " />\n";
echo "<input type=\"checkbox\" disabled=\"disabled\" ";
echo checker($row['publish_all']) . " />\n";
echo "<input type=\"checkbox\" disabled=\"disabled\" ";
echo checker($row['admin']) . " />\n</td>\n";
...
foreach ($cats as $showcat) {
    ...
    //show category
    echo "<tr>\n<td>\n";
    echo "<input class=\"cat\" type=\"text\" value=\"" . 
         htmlentities(urldecode($cats[$i]['name']), ENT_QUOTES, "UTF-8");
