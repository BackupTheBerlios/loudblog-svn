...
if (isset($_POST['commentpreview'])) {
    ...
    $comments[$i]['name'] = htmlentities(strip_tags($_POST['commentname']), ENT_QUOTES, "UTF-8"); 
    }
    $comments[$i]['mail'] = strip_tags($_POST['commentmail']);
    $comments[$i]['web'] = strip_tags($_POST['commentweb']);
    
    $comments[$i]['ip'] = $_SERVER['REMOTE_ADDR'];
    $comments[$i]['message_input'] = $_POST['commentmessage'];
    $comments[$i]['message_html'] = "<p>[PREVIEW]</p> ".no_amp(makehtml(htmlentities($_POST['commentmessage'], ENT_QUOTES, "UTF-8")));
...
if (isset($_POST['commentname'])) { 
    $value = trim(change_entities(stripslashes($_POST['commentname'])));
} else { $value = ""; }
...
if (isset($_POST['commentmessage'])) { 
    $value = trim(htmlentities(stripslashes($_POST['commentmessage']), ENT_QUOTES, "UTF-8")); 
} else { $value = ""; }
return "<textarea name=\"commentmessage\" id=\"commentmessage\">".$value."</textarea>";
}
